...
namespace DynamicSepticSystem
{
    public static class Actualizador
    {
        ...
        private static readonly string VersionHardcoded = "1.9.1-I";
        ...
        // Carpeta de servidor (lee de App.config; si no, fallback UNC)
        private static readonly string carpetaServidor =
            ConfigurationManager.AppSettings["CarpetaActualizaciones"]
        ...
        public static async Task<(bool Success, string Message)> TestConectividadAsync()
        {
            ...
            try
            {
                using (var http = new HttpClient())
                {
                    ...
                    if (!resp.IsSuccessStatusCode)
                    {
                        ...
                        return (false, "No hay conexión a internet disponible.");
                    }
                    Log("Test 1 OK: Conexión a internet disponible");
                }
            }
            catch (Exception ex)
            {
                Log("Test 1 ERROR: " + ex.Message);
                return (false, "Error al verificar conexión a internet: " + ex.Message);
            }
            ...
            try
            {
                using (var http = new HttpClient())
                {
                    ...
                    if (resp.StatusCode == System.Net.HttpStatusCode.NotFound)
                    {
                        ...
                        return (false, $"El repositorio {owner}/{repo} no existe o no es público.");
                    }
                    
                    if (!resp.IsSuccessStatusCode)
                    {
                        Log($"Test 2 FALLIDO: GitHub API devolvió {resp.StatusCode}");
            ...
            try
            {
                using (var http = new HttpClient())
                {
                    ...
                    {
                        Log("Test 3 FALLIDO: Respuesta vacía o inválida de la API");
                        return (false, "La respuesta de GitHub API es inválida.");
                    }
        ...
        private static async Task<(string Tag, string VersionText, string ZipUrl)> GetLatestReleaseInfoAsync()
        {
            var owner = ConfigurationManager.AppSettings["GitHubOwner"] ?? "LuxuImNot";
            var repo = ConfigurationManager.AppSettings["GitHubRepo"] ?? "CalandriaApp";
            ...
            using (var http = new HttpClient())
            {
                ...
                if (!string.IsNullOrWhiteSpace(token))
                {
                    ...
                    Log("Token de autenticación configurado");
                }
                ...
                Log($"Tag extraído: {tag ?? "(ninguno)"}");

                // Extraer asset version.txt si existe
                var versionMatch = Regex.Match(json, "\"name\"\\s*:\\s*\"version.txt\"[\\s\\S]*?\"browser_download_url\"\\s*:\\s*\"([^\"]+)\"", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                string versionUrl = null;
                if (versionMatch.Success)
                    versionUrl = versionMatch.Groups[1].Value.Replace("\\/", "/");

                Log($"URL de version.txt: {versionUrl ?? "(no encontrado)"}");
                ...
                if (!string.IsNullOrWhiteSpace(versionUrl))
                {
                    try
                    {
                        using (var http2 = new HttpClient())
                        {
                            ...
                            http2.DefaultRequestHeaders.UserAgent.ParseAdd("CalandriaUpdater/1.0");
                            if (!string.IsNullOrWhiteSpace(token))
                            {
                                try { http2.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("token", token); } catch { }
                            }
                            var resp = await http2.GetAsync(versionUrl);
                            if (resp.IsSuccessStatusCode)
                            {
                                versionText = (await resp.Content.ReadAsStringAsync()).Trim();
                                Log($"version.txt descargado: {versionText}");
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        Log("Error descargando version.txt desde asset: " + ex.Message);
                    }
                }
                ...
                // Si no se pudo obtener versionText, usar tag como fallback
                if (string.IsNullOrWhiteSpace(versionText) && !string.IsNullOrWhiteSpace(tag))
                {
                    versionText = tag;
                    Log($"Usando tag como versión: {versionText}");
                }
        ...
        // --------------------------------------------------
        // Verificar y actualizar: descarga version.txt desde GitHub, compara y si es necesario descarga ZIP y extrae
        public static async Task<bool> VerificarYActualizarAsync()
        {
            ...
            try
            {
                ...
                Log($"Versión local actual: {VersionLocal}");
                Log("========================================");
                ...
                {
                    MessageBox.Show($"Fallo en test de conectividad:\n\n{testResult.Message}\n\nRevisa tu conexión a internet y la configuración del repositorio.", 
                ...
                // Paso 2: Obtener info de la última release desde la API
                Log("Paso 2: Obteniendo información de la última release...");
                ...
                {
                    MessageBox.Show("No se pudo obtener información de la release más reciente desde GitHub.\n\nVerifica que:\n- El repositorio exista\n- Haya releases publicadas\n- La configuración de GitHubOwner/GitHubRepo sea correcta", 
                        "Actualización", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Log("GetLatestReleaseInfoAsync devolvió información vacía");
                    return false;
                }

                versionRemotaRaw = latest.VersionText;
                Log($"Versión remota detectada: {versionRemotaRaw}");

                var local = VersionLocal;
                var remotaParaMostrar = versionRemotaRaw;

                // Paso 3: Comparación personalizada
                Log("Paso 3: Comparando versiones...");
                bool esNueva = EsNuevaVersion(local, versionRemotaRaw);
                Log($"¿Es nueva versión? {esNueva} (Local: {local}, Remota: {versionRemotaRaw})");
                
                if (!esNueva)
                {
                    MessageBox.Show($"Tu versión ({local}) ya está actualizada.\n\nVersión disponible: {remotaParaMostrar}", 
                        "Actualización", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Log($"No hay actualización necesaria");
                    return false;
                }
                ...
                {
                    MessageBox.Show("No se encontró un paquete ZIP para la release más reciente.\n\nAsegúrate de subir un archivo .zip como asset en la release.",
                        "Actualización", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Log("No se encontró URL de ZIP para descargar");
                    return false;
                }
                ...
                {
                    MessageBox.Show("La URL de actualización no está permitida por seguridad.\n\nLa operación se canceló.",
                        "Actualización bloqueada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                Log("Paso 5: Descargando paquete de actualización...");
                ...
                try
                {
                    using (var http = new HttpClient())
                    {
                        ...
                        using (var resp = await http.GetAsync(updateUrl, HttpCompletionOption.ResponseHeadersRead))
                        {
                            ...
                            Log($"Tamaño del paquete: {(total > 0 ? (total / 1024.0 / 1024.0).ToString("F2") + " MB" : "desconocido")}");
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show("Error al descargar el paquete de actualización:\n\n" + ex.Message + "\n\nIntenta nuevamente más tarde.", 
                ...
                if (!string.IsNullOrEmpty(hashEsperado))
                {
                    if (!VerificarSha256(tempZip, hashEsperado))
                    {
                        ...
                        MessageBox.Show("El paquete de actualización no pasó la verificación de integridad (SHA-256).\n\nNo se aplicará la actualización.",
                            "Actualización bloqueada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                {
                    try { File.Delete(tempZip); } catch { }
                    MessageBox.Show("No se encontró el hash de verificación del paquete y la política de seguridad exige verificarlo.\n\nActualización cancelada.",
                        "Actualización bloqueada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                try
                {
                    ...
                    Log($"Directorio de instalación: {destino}");

                    // Crear carpeta temporal dentro del directorio de la aplicación
                    ...
                    // Escribir version.txt en la carpeta temporal para que el Updater lo copie
                    try
                    {
                        File.WriteAllText(Path.Combine(tempDir, "version.txt"), remotaParaMostrar ?? string.Empty);
                        Log("version.txt escrito en carpeta temporal");
                    }
                    catch { }

                    // Intentar lanzar Updater.exe desde la carpeta temporal con elevación
                    var updaterPath = Path.Combine(tempDir, "Updater.exe");
                    if (File.Exists(updaterPath))
                    {
                        Log($"Updater.exe encontrado: {updaterPath}");

                        // Verificación de firma digital ANTES de ejecutar con privilegios
                        ...
                        if (!string.IsNullOrWhiteSpace(thumbprintFirma))
                        {
                            if (!FirmaConfiable(updaterPath, thumbprintFirma))
                            {
                                ...
                                MessageBox.Show("La actualización no pasó la verificación de firma digital.\n\nNo se aplicará por seguridad.",
                                    "Actualización bloqueada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        {
                            Log("UpdateSignerThumbprint no configurado; se omite verificación de firma (se recomienda configurarlo).");
                        }

                        // Mostrar mensaje al usuario ANTES de solicitar elevación
                        var confirmResult = MessageBox.Show(
                            $"Se descargó la actualización a la versión {remotaParaMostrar}.\n\n" +
                            "Para aplicar la actualización, necesitamos permisos de administrador.\n\n" +
                            "¿Deseas continuar?\n\n" +
                            "Se cerrará la aplicación y se solicitarán permisos de administrador.",
                        ...
                        {
                            Log("Usuario canceló la aplicación de actualización");
                            MessageBox.Show(
                                $"La actualización fue descargada pero no se aplicó.\n\n" +
                                $"Puedes aplicarla más tarde ejecutando manualmente:\n{updaterPath}\n\n" +
                                "Asegúrate de ejecutarlo como Administrador (clic derecho > Ejecutar como administrador)",
                                "Actualización Cancelada",
                        ...
                        try
                        {
                            ...
                            MessageBox.Show(
                                $"Actualización iniciada correctamente.\n\n" +
                                $"Se aplicará la versión {remotaParaMostrar}.\n\n" +
                                "La aplicación se cerrará ahora para completar la instalación.",
                            ...
                            // Salir de la aplicación para permitir que el updater reemplace archivos
                        ...
                        catch (System.ComponentModel.Win32Exception ex)
                        {
                            ...
                            MessageBox.Show(
                                "No se pudo iniciar el actualizador porque se canceló la elevación de privilegios o no tienes permisos de administrador.\n\n" +
                                "Para aplicar la actualización manualmente:\n\n" +
                                ...
                                "La actualización quedará pendiente hasta que completes estos pasos.",
                    ...
                    Log("Intentando aplicar actualización sin Updater.exe...");
                    ...
                    try
                    {
                        ...
                        // Si se copiaron correctamente o se programaron reemplazos, escribir version.txt en destino
                        try
                        {
                            var versionDest = Path.Combine(destino, "version.txt");
                            File.WriteAllText(versionDest, remotaParaMostrar ?? string.Empty);
                            Log("version.txt escrito en destino");
                        }
                        catch { }

                        if (failed.Count == 0)
                        {
                            MessageBox.Show($"Actualización aplicada correctamente.\n\nSi hay archivos en uso se aplicarán al reiniciar el sistema.\n\nVersión: {remotaParaMostrar}", 
                                "Actualización Completada", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Log($"Actualización completada sin fallos: {remotaParaMostrar}");
                            Log("========================================");
                            return true;
                        }
                        else
                    ...
                    catch (Exception exCopy)
                    {
                        ...
                        MessageBox.Show($"La actualización se extrajo en:\n{tempDir}\n\nError al copiar archivos: {exCopy.Message}\n\nEjecuta Updater.exe manualmente desde esa carpeta con permisos de administrador.", 
                            "Error al Aplicar Actualización", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return true;
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"La actualización se descargó pero no se pudo preparar:\n\n{ex.Message}\n\nRevisa los logs en Actualizador.log para más detalles.", 
                        "Error de Preparación", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                try { if (frmProgreso != null && !frmProgreso.IsDisposed) frmProgreso.Invoke((Action)(() => frmProgreso.Close())); } catch { }
                MessageBox.Show($"Error general en el proceso de actualización:\n\n{ex.Message}\n\nRevisa Actualizador.log para detalles completos.", 
        ...
        private static void CopiarConReintentos(string origen, string destino, int reintentos, int esperaMs)
        {
            ...
            {
                Log($"CopiarConReintentos último intento falla para {origen} -> {destino}: {ex}");
                return;
            }
        }
        ...
        private static string ObtenerVersionServidor()
        {
            try
            {
                string ruta = carpetaServidor;

                if (File.Exists(Path.Combine(ruta, "force.update")))
                    return "9999.0.0.0";

                string versionFile = Path.Combine(ruta, "version.txt");
